...
for pageNo in range(url_indexPage): 
    ...
    index_df = index_df.append(index_data, ignore_index=False, verify_integrity=False)
...
for i in range(len(index_df)): 
    ...
    df = df.append(data, ignore_index=False, verify_integrity=False)
    
    #取出英文字
    english_words = re.findall(r'[a-zA-Z]+', article_body)
    data_en = pd.DataFrame({'english_words': english_words})
    df_en = df_en.append(data_en, ignore_index=False, verify_integrity=False)
